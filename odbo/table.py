...
class MySQLTable(_Table):
    ...
    def get_indexes(self):
        ...
        sql_query = dedent(f"""\
            ...
            WHERE table_schema = '{db_opts.schema}'
    ...
    def create_indexes(self, index_commands):
        ...
        for index_name, index_command in zip(valid_indexes, index_commands):
            ...
            if not isinstance(columns, (list, tuple)):
                columns = [columns]
            sql_command = ("create {unique} index {index_name} on {tablename} ({columns});".format(
    ...
    def compress(self):
        ...
        # Recreate indexes
        system_command = f"myisamchk -rq --sort-index --analyze '{index_file}'"
        ...
        logger.info("File size before: {:,.2f} MB".format(file_size_before))
        logger.info("File size after: {:,.2f} MB".format(file_size_after))
        logger.info("File size savings: {:,.2f} MB ({:.2f} %)"
    ...
    def compress_all(self):
        ...
        data_files = [
            ...
            for f in os.listdir(op.join(self.datadir, self.name)) if op.splitext(f)[-1] == '.MYI'
        ]
